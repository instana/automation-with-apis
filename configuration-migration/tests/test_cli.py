...
class TestCLI:
    ...
    @patch('cli.Config.from_args')
    @patch('cli.argparse.ArgumentParser.parse_args')
    def test_main_infrastructure_smart_alerts_command(self, mock_parse_args, mock_config_from_args, mock_exit):
        """Test main function with infrastructure-smart-alerts command."""
        mock_parse_args.return_value = _mock_args(
        ...
        mock_migrator = MagicMock()
        mock_migrator.migrate.return_value = {
            "source": 2, "migrated": 2, "updated": 0, "skipped_identical": 0, "skipped_user": 0, "skipped_invalid": 0, "failed": 0
        }
        mock_class = MagicMock(return_value=mock_migrator)

        mock_module = MagicMock()
        mock_module.InfrastructureSmartAlertsMigrator = mock_class
        with patch.dict('sys.modules', {'migrator': mock_module}):
            main()
    ...
    def test_main_infrastructure_smart_alerts_command_no_migration(self, mock_parse_args, mock_config_from_args, mock_exit):
        ...
        mock_migrator = MagicMock()
        mock_migrator.migrate.return_value = {
            "source": 2, "migrated": 0, "updated": 0, "skipped_identical": 2, "skipped_user": 0, "skipped_invalid": 0, "failed": 0
        }
        mock_class = MagicMock(return_value=mock_migrator)
